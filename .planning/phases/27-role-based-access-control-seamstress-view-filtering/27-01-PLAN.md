...
2. At component top: `const { currentStaff, isLoading: isStaffLoading } = useStaffSession();`
3. Derive: `const isSeamstress = currentStaff?.staffRole === 'seamstress';`
4. **CRITICAL: Gate `fetchOrders` on `!isStaffLoading`.** During localStorage hydration the session is null and `isSeamstress` would be `false`, causing a double-fetch (first unfiltered, then filtered once session resolves). Add `isStaffLoading` to the `fetchOrders` dependency array and return early at the top of `fetchOrders` if `isStaffLoading` is true. This prevents fetching orders before the staff session is resolved.
5. In the `fetchOrders` function (or wherever `/api/orders` is called), append `&seamstressId=${currentStaff.staffId}` to the URL when `isSeamstress && currentStaff?.staffId`
...
2. Read the modified files to confirm:
   ...
   - Board page destructures `isLoading` from `useStaffSession()` and gates `fetchOrders` on `!isStaffLoading`
...
- Board page gates `fetchOrders` on `!isStaffLoading` to prevent double-fetch during session hydration
