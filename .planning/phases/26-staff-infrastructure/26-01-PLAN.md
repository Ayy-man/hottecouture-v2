...
3. Fix inconsistent GET endpoint in `src/components/team/team-management-modal.tsx`:
   - On line 93, the `fetchStaff` function calls `fetch('/api/admin/team')` for the GET request, while all write operations (POST/PATCH/DELETE at lines 122, 155, 182, 206) already use `/api/staff`.
   - Change line 93 from:
     ```typescript
     ...
     ```
   - This makes ALL fetch calls in the modal consistently use `/api/staff`, avoiding a split where reads go to one endpoint and writes go to another. Both endpoints return identical results (since `/api/staff` re-exports from `/api/admin/team`), but consistency prevents confusion and future breakage if the admin endpoint is ever removed.

4. Remove chatbot from `src/app/layout.tsx`:
  ...
  <verify>
    ...
    - Verify `team-management-modal.tsx` line 93 fetches from `/api/staff` (not `/api/admin/team`)
