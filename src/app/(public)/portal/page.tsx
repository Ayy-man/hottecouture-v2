...
const STAGE_ICONS: Record<string, React.ReactNode> = {
  ...
  working: <Scissors className="h-4 w-4" />,
  done: <CheckCircle className="h-4 w-4" />,
  ready: <Gift className="h-4 w-4" />,
...
export default function CustomerPortalPage() {
  ...
  function buildTimelineItems(order: OrderResult): TimelineItem[] {
    ...
    return ORDER_STAGES.map((stage, index) => {
      ...
      const title = t(`stages.${stage}` as Parameters<typeof t>[0]);
      ...
      if (index < currentStageIndex) {
        ...
        if (stage === 'ready' && order.due_date) {
          date = new Date(order.due_date).toLocaleDateString(locale === 'fr' ? 'fr-CA' : 'en-CA', {
            day: 'numeric',
            month: 'short',
          });
        } else {
      ...
      <div className='h-full flex flex-col overflow-hidden'>
        <div className='flex-1 overflow-y-auto'>
            ...
            {!selectedOrder ? (
              ...
              <div className='space-y-6'>
                  ...
                  <div>
                    ...
                    <div className='flex items-center gap-3 mt-1.5 text-sm text-muted-foreground'>
                      <span className='flex items-center gap-1'>
                        ...
                        {selectedOrder.garment_count} {t('items')}
                      </span>
                ...
                <div className='bg-primary-50/50 rounded-xl p-4 border border-primary-100'>
                  <h3 className='font-semibold text-foreground mb-4'>{t('orderTracking')}</h3>
          ...
          <div className='text-center mt-6 text-muted-foreground text-sm animate-fade-in-up-delay-2'>
            <p>{t('contactUs')} <a href={`tel:${SHOP_PHONE.replace(/-/g, '')}`} className='underline text-primary-600 hover:text-primary-700'>{SHOP_PHONE}</a></p>
          </div>
          </div>
        </div>
      </div>
