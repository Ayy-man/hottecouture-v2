        ...
        for (const service of garment.services) {
          ...
          if (isCustomService) {
            ...
            try {
              ...
              // Use user-provided time estimate, or default to 30 min per quantity
              const customEstimatedMinutes = service.estimatedMinutes
            ...
            // Use user-provided time estimate if available, otherwise calculate from pricing model
            const estimatedMinutes = service.estimatedMinutes
              ? service.estimatedMinutes
              : (existingService as any).pricing_model === 'hourly'
