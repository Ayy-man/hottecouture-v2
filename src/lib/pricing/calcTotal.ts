...
export function getPricingConfig(): PricingConfig {
  return {
    rush_fee_small_cents: parseInt(
      process.env.RUSH_FEE_SMALL_CENTS || '3000',
      10
    ),
    rush_fee_large_cents: parseInt(
      process.env.RUSH_FEE_LARGE_CENTS || '6000',
      10
    ),
...
export function calculateOrderPricing(
  ...
  const taxable_amount = subtotal_cents + rushFee.rush_fee_cents;
...
export function validatePricingConfig(config: PricingConfig): {
  ...
  if (config.rush_fee_large_cents < config.rush_fee_small_cents) {
    errors.push(
      'Rush fee large must be greater than or equal to rush fee small'
    );
  }

  if (config.gst_pst_rate_bps < 0 || config.gst_pst_rate_bps > 10000) {
    errors.push(
      'GST/PST rate must be between 0 and 10000 basis points (0-100%)'
    );
  }
...
export function getPricingSummary(calculation: PricingCalculation): {
  ...
  const breakdown = {
    subtotal_percentage: calculatePercentage(
      calculation.subtotal_cents,
      calculation.total_cents
    ),
    rush_fee_percentage: calculatePercentage(
      calculation.rush_fee_cents,
      calculation.total_cents
    ),
    tax_percentage: calculatePercentage(
...
export function recalculateOrderPricing(
  ...
  return {
    ...
    tps_cents: calculation.tps_cents,
    tvq_cents: calculation.tvq_cents,
