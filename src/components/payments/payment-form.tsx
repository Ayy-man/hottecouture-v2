...
export function PaymentForm({
  ...
  const selectedProviderData = paymentProviders.find(
  ...
  const paymentFees = selectedMethodData
    ? calculatePaymentFees(
        paymentAmount,
        selectedMethodData,
        DEFAULT_PAYMENT_CONFIG.taxRate
      )
    : null;
    ...
    try {
      const response = await fetch('/api/payments/create-intent', {
        ...
        body: JSON.stringify({
          ...
          metadata: {
            paymentType:
              paymentAmount === summary.totalAmount ? 'full' : 'deposit',
    ...
    <div className={`space-y-6 ${className}`}>
        ...
        <CardContent>
          <div className='space-y-3'>
            ...
            <div className='text-sm text-muted-foreground'>
              {t('depositRequiredLabel')} {formatPaymentAmount(summary.depositAmount)} (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t('methodDescription')}</CardDescription>
        </CardHeader>
        <CardContent>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            {paymentMethods
        ...
        <CardContent>
          <div className='space-y-4'>
            <div className='grid grid-cols-2 gap-4'>
              <Button
                variant={
                  paymentAmount === summary.depositAmount
                    ? 'default'
                    : 'outline'
                }
              ...
              >
                <div className='text-center'>
                  <div className='font-bold'>{t('depositOnly')}</div>
              ...
              <Button
                variant={
                  paymentAmount === summary.totalAmount ? 'default' : 'outline'
                }
                onClick={() => setPaymentAmount(summary.totalAmount)}
                className='h-16'
              >
                <div className='text-center'>
                  <div className='font-bold'>{t('payFull')}</div>
                  <div className='text-sm'>
      ...
      >
        {isProcessing ? (
          ...
          t('pay', { amount: formatPaymentAmount(paymentFees?.total || paymentAmount) })
        )}
      </Button>
    </div>
  );
