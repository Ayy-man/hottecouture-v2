...
export function ServicesStepNew({
  ...
  const handleCreateCategory = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        alert(result.error || t('categoryCreateFailed'));
        return;
      }
  ...
  const handleSaveEditCategory = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        alert(result.error || t('categoryUpdateFailed'));
        return;
      }
  ...
  const handleDeleteCategory = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        alert(result.message || result.error || t('categoryDeleteFailed'));
  ...
  const handleCreateService = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        alert(result.error || t('serviceCreateFailed'));
        return;
      }
  ...
  const handleSaveEditService = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        alert(result.error || t('serviceUpdateFailed'));
        return;
      }
  ...
  const handleDeleteService = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        alert(result.message || result.error || t('serviceDeleteFailed'));
        ...
        <div className='flex-1 text-center'>
          ...
          {client ? (
            <p className='text-sm text-muted-foreground'>
              {t('addingServicesTo')} {client.first_name} {client.last_name}{' '}
              {onChangeCustomer && (
                <button
            ...
            <p className='text-sm text-muted-foreground'>{t('chooseDescription')}</p>
          )}
        </div>
        ...
        >
          ...
          {!showAddCategoryForm && categoriesCount < 8 && (
            ...
            >
              ...
              <span className='text-xs leading-tight text-center'>{tc('add')}</span>
            </button>
          )}
      ...
      {deleteConfirmId && (
        <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 animate-[fadeIn_0.2s_ease-out]'>
          <div className='bg-white rounded-lg p-6 max-w-md w-full mx-4 animate-[fadeIn_0.2s_ease-out,zoomIn_0.2s_ease-out]'>
            <h3 className='text-lg font-semibold mb-4'>{t('deleteCategoryTitle')}</h3>
              ...
              <p className='text-sm text-muted-foreground mb-4'>
                {categoryUsageCount > 0 ? (
                  <span className='text-red-600 font-semibold'>
                    {t('categoryInUse', { count: categoryUsageCount })}
                  </span>
                ) : (
                  <span className='text-green-600'>
                    {t('categorySafeToDelete')}
                  </span>
                )}
              </p>
            )}
      ...
      <div className='flex-1 flex overflow-hidden min-h-0'>
                              ...
                              <div className='mt-auto pt-2'>
                                {(() => {
                                  ...
                                  return (
                                    ...
                                    >
                                      {isAdded
                                        ? t('addToGarmentAdded', { index: String(activeGarmentIndex + 1), qty: String(garmentService?.qty || 0) })
                                        : t('addToGarment', { index: String(activeGarmentIndex + 1) })}
                                    </button>
                                  );
                                })()}
                              </div>
                  ...
                  <div className='bg-white rounded-lg p-6 max-w-md w-full mx-4 animate-[fadeIn_0.2s_ease-out,zoomIn_0.2s_ease-out]'>
                    ...
                    {serviceUsageCount !== null && (
                      <p className='text-sm text-muted-foreground mb-4'>
                        {serviceUsageCount > 0 ? (
                          <span className='text-red-600 font-semibold'>
                            {t('serviceInUseCount', { count: serviceUsageCount })}
                          </span>
                        ) : (
                          <span className='text-green-600'>
                            {t('serviceSafeToDelete')}
                          </span>
                        )}
                      </p>
                    )}
                ...
                <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 animate-[fadeIn_0.2s_ease-out]'>
                  <div className='bg-white rounded-lg p-6 max-w-lg w-full mx-4 max-h-[80vh] flex flex-col animate-[fadeIn_0.2s_ease-out,zoomIn_0.2s_ease-out]'>
                              ...
                              <div className='flex-1 min-w-0'>
                                ...
                                <p className='text-xs text-muted-foreground mt-0.5'>
                                  {(zipService as any).unit &&
                                    `${t('unitPrefix')} ${(zipService as any).unit}`}
                                </p>
                              </div>
                    ...
                    <div className='flex gap-2 pt-4 border-t border-border'>
                      ...
                      >
                        {selectedZipService ? t('withZip') : t('addTheService')}
                      </button>
                    </div>
                  </div>
                </div>
        ...
        <div className='w-80 bg-white border-l border-border flex flex-col h-full'>
          <div className='p-4 border-b border-border flex-shrink-0'>
            <h3 className='font-semibold text-foreground'>{t('selectedServices')}</h3>
