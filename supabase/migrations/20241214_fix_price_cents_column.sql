-- FINAL FIX: Ensure get_orders_with_details uses total_cents not price_cents
-- This migration runs after all previous migrations to guarantee the fix

-- Drop and recreate the function with the correct column name (total_cents)
...
RETURNS TABLE (
  ...
  total_cents BIGINT,  -- CORRECT: Using total_cents, not price_cents
  is_active BOOLEAN,
  -- Client fields
...
BEGIN
  ...
  SELECT
    ...
    o.total_cents,  -- CORRECT: Using total_cents, not price_cents
    o.is_active,
  ...
  GROUP BY
    ...
    o.total_cents, o.is_active,  -- CORRECT: Using total_cents
...
-- Verification query - this should work without errors
