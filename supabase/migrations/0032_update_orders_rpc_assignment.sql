-- Migration: Update get_orders_with_details RPC to include item-level assignment
-- Purpose: Return assigned_seamstress_id and assigned_seamstress_name in services JSON,
-- and add optional filter parameter for filtering by seamstress.
...
CREATE OR REPLACE FUNCTION get_orders_with_details(
  ...
  p_assigned_seamstress_id UUID DEFAULT NULL  -- NEW: filter orders with items assigned to this seamstress
)
RETURNS TABLE (
  id UUID,
  order_number BIGINT,
